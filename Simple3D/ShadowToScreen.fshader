...
float CalcShadow2(){
  ...
  if (texture(ShadowMap, fDepthPosition.xy + vec2(-0.000015, 0.000005)).r < fDepthPosition.z - bias){
    V -= 0.2;
  }
  if (texture(ShadowMap, fDepthPosition.xy + vec2(0.000005, 0.00005) ).r < fDepthPosition.z - bias){
    V -= 0.2;
  }
  if (texture(ShadowMap, fDepthPosition.xy + vec2(-0.000015, -0.00015) ).r < fDepthPosition.z - bias){
    V -= 0.2;
  }
  if (texture(ShadowMap, fDepthPosition.xy + vec2(0.000005, -0.000015)).r < fDepthPosition.z - bias){
    V -= 0.2;
  }
...
void main(){
  ...
  //Diffuse
  float Diff = clamp(dot(fNormal, fLightDir),0, 1);
