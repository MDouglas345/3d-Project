...
//Every object will use their own VAO.. save on bandiwidth and improve performance.
VanillaRenderer::VanillaRenderer(int Width, int Height)
{
	...
	this->BasicShaderProgram = Util.LoadShaders("VertexV2.vshader", "FragmentV2.fshader");
	this->ShadowShaderProgram = Util.LoadShaders("ShadowVertex.vshader", "ShadowFragment.fshader");
	this->dBasicWShadowProgram = Util.LoadShaders("VertexV2wShadow.vshader", "FragmentV2wShadow.fshader");
	...
	this->DebugQuadShaderProgram = Util.LoadShaders("QuadDebugVertex.vshader", "QuadDebugFragment.fshader");
	this->AntonsQuad = Util.LoadShaders("ss_quad.vert", "ss_quad.frag");

	this->DebugShadowToTextureProgram = Util.LoadShaders("DepthToTexture.vshader", "DepthToTexture.fshader");
	this->DebugShadowTextureToScreenProgram = Util.LoadShaders("ShadowToScreen.vshader", "ShadowToScreen.fshader");

	this->ContactPointShaderProgram = Util.LoadShaders("ContactPoint.vshader", "ContactPoint.fshader");
...
void VanillaRenderer::DebugShadowToTexture(const std::vector<Object*> Objects)
{
	...
	for (int x = 0; x < Objects.size(); x++) {
		...
		GLuint m_V_ID = glGetUniformLocation(DebugShadowTextureToScreenProgram, "V");
		//GLuint m_V_ID = glGetUniformLocation(dBasicWShadowProgram, "V");
...
void VanillaRenderer::DebugRenderContactPoints(const std::vector<glm::vec3> Contacts)
{
	glUseProgram(ContactPointShaderProgram);
	...
	for (int i = 0; i < Contacts.size(); i++) {
		...
		glGenBuffers(1, &VertexBuffer);
		glBindBuffer(GL_ARRAY_BUFFER, VertexBuffer);
		glBufferData(GL_ARRAY_BUFFER,  sizeof(glm::vec3), &Contacts[i], GL_STATIC_DRAW);

		GLuint ColorID = glGetUniformLocation(ContactPointShaderProgram, "Color");
		GLuint MVPID = glGetUniformLocation(ContactPointShaderProgram, "MVP");

		glm::mat4 MVP = Projection * View * glm::translate(glm::mat4(), Contacts[i]);
